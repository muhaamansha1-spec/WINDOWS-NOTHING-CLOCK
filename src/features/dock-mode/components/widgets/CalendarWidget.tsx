import { memo } from 'react';
import { motion } from 'framer-motion';
import { Typography, Icon } from '../../../../components/ui';
import type { WidgetData } from '../../types';

export const CalendarWidget = memo(function CalendarWidget({ 
  data, 
  brightness = 1,
  burnInOffset = { x: 0, y: 0 },
}: { 
  data: WidgetData['calendar']; 
  brightness?: number;
  burnInOffset?: { x: number; y: number };
}) {
  const style = { opacity: brightness, transform: `translate(${burnInOffset.x}px, ${burnInOffset.y}px)` };

  if (!data || data.length === 0) {
    return (
      <div style={style} className="flex items-center justify-center h-full text-white/30">
        <Typography variant="body-sm" className="font-mono">No upcoming events</Typography>
      </div>
    );
  }

  return (
    <div style={style} className="space-y-3">
      {data.slice(0, 4).map((event, index) => (
        <motion.div
          key={index}
          className="flex items-center gap-3 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: index * 0.05 }}
        >
          <div className="w-2 h-2 rounded-full bg-white/40 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <Typography variant="body-sm" className="text-white truncate font-mono">
              {event.title}
            </Typography>
            <Typography variant="caption" className="text-white/40 font-mono">
              {event.time} {event.allDay ? '• All day' : ''}
            </Typography>
          </div>
        </motion.div>
      ))}
    </div>
  );
});

CalendarWidget.displayName = 'CalendarWidget';